...
# Function to zip files or directories
function zip_files {
    echo "Enter the file(s) or directory you want to zip (space-separated if multiple):"
    read -r files
    echo "Enter the name for the output zip file (e.g., archive.zip):"
    read -r zip_name

    zip -r "$zip_name" $files
    if [ $? -eq 0 ]; then
        echo "Zipping successful. Created file: $zip_name"
    else
        echo "Zipping failed. Please check the input files or directories."
    fi
}

# Function to unzip files
function unzip_files {
    echo "Enter the path of the zip file to unzip:"
    read -r zip_file
    ...
    echo "Enter the directory to extract files to (or press Enter for the current directory):"
    read -r extract_dir

    unzip "$zip_file" -d "${extract_dir:-.}"
    if [ $? -eq 0 ]; then
        echo "Unzipping successful. Extracted to: ${extract_dir:-current directory}"
    else
        echo "Unzipping failed. Please check the zip file."
    fi
}
...
# Main menu
# Add options for zip and unzip in the main menu
function main_menu {
    ...
    echo "13. Zip Files or Directories"
    echo "14. Unzip a File"
    echo "15. Quit"
    read -p "Enter your choice: " choice

    case "$choice" in
        1) display_system_info ;;
        3) display_directory_structure ;;
        4) kill_process ;;
        5) delete_directory ;;
        6) display_network_info ;;
        7) display_hardware_info ;;
        8) encrypt_file ;;
        9) decrypt_file ;;
        10) monitor_process ;;
        11) system_health_check ;;
        12) security_audit ;;
        13) zip_files ;;          # New option for zipping
        14) unzip_files ;;        # New option for unzipping
        15) exit 0 ;;
